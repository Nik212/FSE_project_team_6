...
def load_hdf5_data(filename, num_classes):
    ...
    with h5py.File(filename, 'r') as _f:
        volumes = _f['data'][:].astype(np.float32)
        labels = _f['label'][:]
        frames = _f['frames'][:]
        world_to_grids = _f['world_to_grid'][:]
...
def load_scene(filename, num_classes, load_gt):
    ...
    sdfs = struct.unpack('f'*num_elems, fin.read(num_elems*4))  #grid3<float>
    labels = None
    if load_gt:
        labels = struct.unpack('B'*num_elems, fin.read(num_elems))  #grid3<uchar>
    fin.close()
...
def load_scene_image_info_multi(filename, scene_name, image_path, depth_image_dims, color_image_dims, color_mean, color_std):
    """
    ...
    num_elems = width * height * max_num_images
    frame_ids = struct.unpack('i'*num_elems, fin.read(num_elems*4))  #grid3<int>
    _width = struct.unpack('<Q', fin.read(8))[0]
    _height = struct.unpack('<Q', fin.read(8))[0]
    assert width == _width and height == _height
    num_elems = width * height * 4 * 4
    world_to_grids = struct.unpack('f'*num_elems, fin.read(num_elems*4))  #grid2<mat4f>
    fin.close()
    ...
    poses = {}
    normalize = transforms.Normalize(mean=color_mean, std=color_std)
    for _f in unique_frame_ids:
        if _f == -1:
            continue
        depth_file = os.path.join(image_path, scene_name, 'depth', str(_f) + '.png')
        color_file = os.path.join(image_path, scene_name, 'color', str(_f) + '.jpg')
        pose_file = os.path.join(image_path, scene_name, 'pose', str(_f) + '.txt')
        ...
        depth_images[_f] = torch.from_numpy(depth_image.astype(np.float32))
        color_images[_f] = color_image
        poses[_f] = pose
