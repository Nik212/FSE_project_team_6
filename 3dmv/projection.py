...
class ProjectionHelper():
    ...
    def depth_to_skeleton(self, u_x, u_y, depth):
        """
        @param u_x:
        @param u_y:
        @param depth:
        @return:
        """
        _x = (u_x - self.intrinsic[0][2]) / self.intrinsic[0][0]
        _y = (u_y - self.intrinsic[1][2]) / self.intrinsic[1][1]
        return torch.Tensor([depth*_x, depth*_y, depth])


    def skeleton_to_depth(self, _p):
        """
        @param _p:
        @return:
        """
        _x = (_p[0] * self.intrinsic[0][0]) / _p[2] + self.intrinsic[0][2]
        _y = (_p[1] * self.intrinsic[1][1]) / _p[2] + self.intrinsic[1][2]
        return torch.Tensor([_x, _y, _p[2]])
    ...
    def compute_frustum_bounds(self, world_to_grid, camera_to_world):
        ...
        @return:
        """
        corner_points = camera_to_world.new(8, 4, 1).fill_(1)  # depth min
        ...
        corner_points[3][:3] = self.depth_to_skeleton(0, self.image_dims[1] - 1, self.depth_min).unsqueeze(1)  # depth max
        ...
        _p = torch.bmm(camera_to_world.repeat(8, 1, 1), corner_points)
        p_l = torch.round(torch.bmm(world_to_grid.repeat(8, 1, 1), torch.floor(_p)))
        p_u = torch.round(torch.bmm(world_to_grid.repeat(8, 1, 1), torch.ceil(_p)))
        bbox_min0, _ = torch.min(p_l[:, :3, 0], 0)
        bbox_min1, _ = torch.min(p_u[:, :3, 0], 0)
        bbox_min = np.minimum(bbox_min0, bbox_min1)
        bbox_max0, _ = torch.max(p_l[:, :3, 0], 0)
        bbox_max1, _ = torch.max(p_u[:, :3, 0], 0)
    ...
    def compute_projection(self, depth, camera_to_world, world_to_grid):
        ...
        _p = torch.mm(world_to_camera, torch.mm(grid_to_world, coords))

        # project into image
        _p[0] = (_p[0] * self.intrinsic[0][0]) / _p[2] + self.intrinsic[0][2]
        _p[1] = (_p[1] * self.intrinsic[1][1]) / _p[2] + self.intrinsic[1][2]
        p_i = torch.round(_p).long()

        valid_ind_mask = torch.ge(p_i[0], 0) * torch.ge(p_i[1], 0) * torch.lt(p_i[0], self.image_dims[0]) * torch.lt(p_i[1], self.image_dims[1])
        if not valid_ind_mask.any():
            # print('error: no valid image indices')
            return None
        valid_image_ind_x = p_i[0][valid_ind_mask]
        valid_image_ind_y = p_i[1][valid_ind_mask]
        ...
        depth_mask = depth_vals.ge(self.depth_min) * depth_vals.le(self.depth_max) * torch.abs(depth_vals - _p[2][valid_ind_mask]).le(self.voxel_size)
...
class Projection(Function):
    ...
    def forward(ctx, label, lin_indices_3d, lin_indices_2d, volume_dims):
        ...
        @param ctx:
        @param label:
        @param lin_indices_3d:
        @param lin_indices_2d:
        @param volume_dims:
        @return:
        """
