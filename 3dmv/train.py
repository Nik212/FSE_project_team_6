...
ENET_TYPES = {
    'scannet': (41, [0.496342, 0.466664, 0.440796], [0.277856, 0.28623, 0.291129])}  # classes, color mean/std  # params
parser = argparse.ArgumentParser()  # data paths
...
parser.add_argument('--class_weight_file', default='', help='path to histogram over classes')  # train params
...
parser.add_argument('--use_proxy_loss', dest='use_proxy_loss', action='store_true')  # 2d/3d
...
parser.add_argument('--depth_max', type=float, default=4.0, help='max depth (in meters)')  # scannet intrinsic params
...
optimizer2d = torch.optim.SGD(model2d_trainable.parameters(), lr=opt.lr, momentum=opt.momentum,
                              weight_decay=opt.weight_decay)
if opt.use_proxy_loss:
    optimizer2dc = torch.optim.SGD(model2d_classifier.parameters(), lr=opt.lr, momentum=opt.momentum,
                                   weight_decay=opt.weight_decay)
...
def train(epoch, iter, log_file, train_file, log_file_2d):
    ...
    for _t, _v in enumerate(indices):
        targets = torch.autograd.Variable(labels[_v].cuda())
        # valid targets
        ...
        if len(maskindices.shape) == 0:
            continue
        transforms = world_to_grids[_v].unsqueeze(1)
        transforms = transforms.expand(batch_size, num_images, 4, 4).contiguous().view(-1, 4, 4).cuda()
        data_util.load_frames_multi(opt.data_path_2d, frames[_v], depth_images, color_images, camera_poses, color_mean,
                                    color_std)

        # compute projection mapping
        proj_mapping = [projection.compute_projection(d, c, t) for d, c, t in
                        zip(depth_images, camera_poses, transforms)]
        ...
        if opt.use_proxy_loss:
            data_util.load_label_frames(opt.data_path_2d, frames[_v], label_images, num_classes)
        ...
        # 2d/3d
        input3d = torch.autograd.Variable(volumes[_v].cuda())
        ...
        if opt.use_proxy_loss:
            ...
            # confusion
            _y = ft2d.data
            _y = _y.view(-1, num_classes)[:, :-1]
            _, predictions = _y.max(1)
        ...
        # confusion
        _y = output.data
        _y = _y.view(_y.nelement() / _y.size(2), num_classes)[:, :-1]
        _, predictions = _y.max(1)
        predictions = predictions.view(-1)
...
def test(epoch, iter, log_file, val_file, log_file_2d):
    ...
    with torch.no_grad():
        ...
        for _t, _v in enumerate(indices):
            ...
            transforms = transforms.expand(batch_size, num_images, 4, 4).contiguous().view(-1, 4, 4).cuda()
            # get 2d data
            data_util.load_frames_multi(opt.data_path_2d, frames[_v], depth_images, color_images, camera_poses,
...
def evaluate_confusion(confusion_matrix, loss, epoch, iter, time, which, log_file):
    ...
    for _c in range(num_classes):
        num = conf[_c, :].sum()
        valids[_c] = -1 if num == 0 else float(conf[_c][_c]) / float(num)
        total_correct += conf[_c][_c]
    ...
    log_file.write(_SPLITTER.join(
        [str(f) for f in [epoch, iter, torch.mean(torch.Tensor(loss)), avg_acc, instance_acc, time]]) + '\n')
    log_file.flush()
...
def main():
    ...
    log_file = open(os.path.join(opt.output, 'log.csv'), 'w', encoding='UTF-8')
    log_file.write(_SPLITTER.join(['epoch', 'iter', 'loss', 'avg acc', 'instance acc', 'time']) + '\n')
    log_file.flush()
    log_file_2d = None
    if opt.use_proxy_loss:
        log_file_2d = open(os.path.join(opt.output, 'log2d.csv'), 'w', encoding='UTF-8')
        log_file_2d.write(_SPLITTER.join(['epoch', 'iter', 'loss', 'avg acc', 'instance acc', 'time']) + '\n')
        log_file_2d.flush()

    has_val = len(val_files) > 0
    if has_val:
        log_file_val = open(os.path.join(opt.output, 'log_val.csv'), 'w', encoding='UTF-8')
        log_file_val.write(_SPLITTER.join(['epoch', 'iter', 'loss', 'avg acc', 'instance acc', 'time']) + '\n')
        log_file_val.flush()
        log_file_2d_val = None
        if opt.use_proxy_loss:
            log_file_2d_val = open(os.path.join(opt.output, 'log2d_val.csv'), 'w', encoding='UTF-8')
